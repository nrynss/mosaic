...
> parcels **P35–P50** are claimed and integrated from that board; this v0.2 board
...
- 2026-07-20 v0.3 increment planned by coordinator — recorded RFC-0005 (interactive operator-driven simulation, opt-in server-side live OpenAI models with a budget governor, and deterministic recurrence awareness) and captured the operator-is-real and live-model sections in `docs/demo-script.md`. Parcels P35–P50 with exclusive ownership, dependencies, acceptance, and waves are published in `docs/handoffs/v0.3-interactive-operator-demo.md` for isolated external-builder claims. Wave A (P35 → P36 contracts) is ready; all invariants keep the deterministic fixture path as default/fallback, the OpenAI key server-only, and every operator action an immutable `executed:false` record.
