// Package store provides the append-only SQLite implementation of Mosaic's
...
import (
	...
	"mosaic.local/mosaic/migrations"
...
var (
	...
	ErrInvalidRecord = errors.New("invalid store record")
)

// Store is a local SQLite implementation of the Mosaic repository contracts.
// It uses one connection because the v0.1 demo is a single application
// instance and SQLite foreign-key settings are connection-local.
type Store struct {
	db *sql.DB
}
...
// Open opens a SQLite database at dsn, applies all ordered migrations, and
// enables the database constraints that make the append log durable.
func Open(ctx context.Context, dsn string) (*Store, error) {
	if strings.TrimSpace(dsn) == "" {
		return nil, fmt.Errorf("%w: SQLite DSN is required", ErrInvalidRecord)
	}
	db, err := sql.Open("sqlite", dsn)
	if err != nil {
		return nil, fmt.Errorf("open SQLite database: %w", err)
	}
	// v0.1 deliberately has one process and one store connection. Besides
	// matching that scope, this ensures PRAGMA foreign_keys applies to all work.
...
// OpenInMemory opens an isolated in-memory Store suitable for tests and local
// deterministic demonstrations that do not need a database file after exit.
...
// SQLDB exposes the underlying database for narrow operational checks. Mosaic
// application code should depend on repository contracts instead.
...
func (s *Store) applyMigrations(ctx context.Context) error {
	...
	for _, entry := range entries {
		...
		err := s.db.QueryRowContext(ctx,
			"SELECT 1 FROM schema_migrations WHERE migration_name = ?", entry.Name(),
		).Scan(&present)
		if err == nil {
			continue
		}
...
func (s *Store) executor(ctx context.Context) (sqlExecutor, error) {
...
// WithinTransaction runs fn in one SQLite transaction. Nested calls join the
// caller's transaction so the canonical-event, projection receipt, and
// checkpoint boundary from RFC-0001 §6.3 remains atomic.
...
func (s *Store) AppendRawEvent(ctx context.Context, event gen.RawEvent) (contracts.RawEventAppendResult, error) {
	...
	_, insertErr := exec.ExecContext(ctx, `INSERT INTO raw_events
...
func (s *Store) FindRawEvent(ctx context.Context, rawEventID string) (gen.RawEvent, error) {
	...
	err = exec.QueryRowContext(ctx, "SELECT record_json FROM raw_events WHERE raw_event_id = ?", rawEventID).Scan(&recordJSON)
	if errors.Is(err, sql.ErrNoRows) {
...
func findExistingRawEvent(ctx context.Context, exec sqlExecutor, sourceID, sourceRecordID, idempotencyKey string) (string, bool, error) {
	query := "SELECT raw_event_id FROM raw_events WHERE source_id = ? AND "
	var value string
	if sourceRecordID != "" {
		query += "source_record_id = ?"
		value = sourceRecordID
	} else {
		query += "source_record_id IS NULL AND idempotency_key = ?"
		value = idempotencyKey
	}
	var rawEventID string
	err := exec.QueryRowContext(ctx, query, sourceID, value).Scan(&rawEventID)
	if errors.Is(err, sql.ErrNoRows) {
		return "", false, nil
	}
	if err != nil {
...
func findExistingLunaResult(ctx context.Context, exec sqlExecutor, rawEventID string) (string, error) {
	var resultID string
	err := exec.QueryRowContext(ctx, `SELECT luna_result_id FROM luna_results
		WHERE raw_event_id = ? ORDER BY rowid DESC LIMIT 1`, rawEventID).Scan(&resultID)
	if errors.Is(err, sql.ErrNoRows) {
		return "", nil
	}
	if err != nil {
...
// The incoming CanonicalSeq is ignored: SQLite's persisted append sequence is
// the only projection/replay order.
...
func (s *Store) appendCanonicalEvent(ctx context.Context, event gen.CanonicalEvent) (gen.CanonicalEvent, error) {
	...
	if _, err := exec.ExecContext(ctx, `INSERT INTO canonical_events
	...
	for _, reference := range stored.IncidentRefs {
		...
		if _, err := exec.ExecContext(ctx, `INSERT INTO canonical_event_incidents
			(canonical_event_id, incident_id) VALUES (?, ?)`, stored.CanonicalEventID, incidentID); err != nil {
...
func nextCanonicalSequence(ctx context.Context, exec sqlExecutor) (int64, error) {
	var current int64
	err := exec.QueryRowContext(ctx, "SELECT seq FROM sqlite_sequence WHERE name = 'canonical_events'").Scan(&current)
	if errors.Is(err, sql.ErrNoRows) {
		return 1, nil
	}
	if err != nil {
...
func (s *Store) ListCanonicalEventsAfter(ctx context.Context, canonicalSeq int64) ([]gen.CanonicalEvent, error) {
	...
	rows, err := exec.QueryContext(ctx, `SELECT canonical_seq, record_json FROM canonical_events
		WHERE canonical_seq > ? ORDER BY canonical_seq ASC`, canonicalSeq)
	if err != nil {
...
func (s *Store) MarkCanonicalEventProjected(ctx context.Context, canonicalEventID string, stateRevision int64) error {
	...
	err = exec.QueryRowContext(ctx, `SELECT state_revision FROM canonical_projection_receipts
		WHERE canonical_event_id = ?`, canonicalEventID).Scan(&existingRevision)
	if err == nil {
	...
	if _, err := exec.ExecContext(ctx, `INSERT INTO canonical_projection_receipts
		(canonical_event_id, state_revision, recorded_at) VALUES (?, ?, ?)`,
...
func (s *Store) AppendLunaResult(ctx context.Context, result gen.LunaResult) error {
	...
	_, err = exec.ExecContext(ctx, `INSERT INTO luna_results
		(luna_result_id, raw_event_id, canonical_event_id, record_json) VALUES (?, ?, ?, ?)`,
...
func (s *Store) ListLunaResults(ctx context.Context) ([]gen.LunaResult, error) {
	...
	rows, err := exec.QueryContext(ctx, "SELECT record_json FROM luna_results")
	if err != nil {
...
func (s *Store) ListModelRuns(ctx context.Context) ([]gen.ModelRun, error) {
	...
	rows, err := exec.QueryContext(ctx, "SELECT record_json FROM model_runs")
	if err != nil {
...
func (s *Store) AppendModelRun(ctx context.Context, run gen.ModelRun) error {
	...
	if _, err := exec.ExecContext(ctx, `INSERT INTO model_runs
		(model_run_id, state_revision, record_json) VALUES (?, ?, ?)`, run.ModelRunID, stateRevision, recordJSON); err != nil {
...
func (s *Store) AppendAuditRecord(ctx context.Context, audit gen.AuditRecord) error {
	...
	if _, err := exec.ExecContext(ctx, "INSERT INTO audit_records (audit_record_id, record_json) VALUES (?, ?)", audit.AuditRecordID, recordJSON); err != nil {
...
func (s *Store) appendJSONRecord(ctx context.Context, table, idColumn, id string, stateRevision int64, record any) error {
	...
	statement := fmt.Sprintf("INSERT INTO %s (%s, state_revision, record_json) VALUES (?, ?, ?)", table, idColumn)
	if _, err := exec.ExecContext(ctx, statement, id, stateRevision, recordJSON); err != nil {
...
func (s *Store) AppendCheckpoint(ctx context.Context, checkpoint gen.Checkpoint) error {
	...
	if _, err := exec.ExecContext(ctx, `INSERT INTO checkpoints
		(checkpoint_id, state_revision, through_canonical_seq, record_json) VALUES (?, ?, ?, ?)`,
...
func (s *Store) LatestCheckpoint(ctx context.Context) (gen.Checkpoint, error) {
	...
	var recordJSON string
	err = exec.QueryRowContext(ctx, `SELECT record_json FROM checkpoints
